...
from project.settings import IS_PROD, IS_TESTING, MEDIA_ROOT, NDS
...
def unimat_prompower_api():
    prompower = Supplier.objects.get(slug="prompower")
    vendors = Vendor.objects.filter(slug="unimat")
    vendor_pp = Vendor.objects.filter(slug="prompower")
    ...
    def add_products():
        ...
        for data_item in data:
            ...
            try:
                i += 1
                if data_item["article"] != None and data_item["article"]!= "":
                    print("!!!!!!!!!!!!!!!!number", i)
                    # основная инфа
